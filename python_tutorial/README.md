...
## Python tutorial
These tutorials are adapted from the [original tutorials](https://github.com/sarah1123/ScientificComputing-APMTH111/tree/main/2022Fall/00_python_tutorial) developed by Yue Sun in 2022.
...
You may have different preferences in terms of how to use Python. One easy option would be to use Google Colab. However, if you prefer to work on the code locally, you need to (properly) install your Python environment. In addition, you need to install Jupyter notebook because it allows interactive editing.
...
>
~~(In addition, I recommend installing Python 3.9 even though the latest is 3.10. This is because some libraries only support up to Python 3.9 for now, like `PyTorch`.~~(Outdated comment)

> ⚠️ **Warning**: The Python version mentioned here (3.9) might be outdated. Please check the [official Python website](https://www.python.org/downloads/) for the latest version before proceeding.
